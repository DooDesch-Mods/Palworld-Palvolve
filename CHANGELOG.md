...
- **Two Pals can fuse in a fight for 60 seconds.** Pick "Fuse with ..." in the evolve wheel and the partner steps out of its ball and merges into your summoned Pal. It costs a Fusion Shard, and the same two Pals wait five minutes before the next fusion.
...
- **The Fusion Altar turns two Pals into one for good.** It is a stone gate over two pedestals, and the two Pals wait there facing each other. The fusion plays as a filmed scene with its own sound: both Pals roar, rise off their pedestals and circle each other in front of the gate until they burst into the new Pal, which lands on the first pedestal and roars. Build it from the Pal section of the build menu, put two of your Pals in, pick "Fusion Altar" in the wheel and choose up to four passives and the gender. The second Pal is used up, and each fusion costs a Fusion Core.
...
- **The fourth move slot is finally on screen.** The extra attack has been handed out for a while and drawn nowhere, because the status screen is built with three rows. A Pal that carries four moves now shows four. Reported by SephVII.
- **A rule can ask for a Lucky Pal.** English uses that one word for two different things: the passive skill, and the shiny Pal itself. Only the passive could be picked, so a rule meant for a shiny failed on a Pal that plainly looked the part. Reported by Aulent.
- **A recipe that names an item the game does not have says so.** One wrong material and that recipe never reaches the Pal Alchemy Workbench, while the item it makes is still registered and still logged. The startup line now names the item, the recipe and the file. Reported by SephVII.

### Fixed

- **The extra move slot says what happened.** With the setting on, some evolutions handed out nothing and said nothing, so the setting looked broken. A Pal with four moves already, or with no move better than the three it has, now says so instead of leaving you counting slots. Reported by SephVII.
...
- **A required passive is matched however the name is spelled.** Whichever mod registers a name first decides its capitalisation for that session, so a Pal carrying the passive could read as one that does not.
...
- **Five Pals showed their internal id instead of a name.** Gumoss Botan, Hangyu, Hangyu Cryst, Snock Ignis and Tanzee Cryst have no name in the game's own text table, so the wheel fell back to `WindChimes_Ice` and the like. They are named from the same source the website uses, in all 17 languages. Reported by iknowimgodly.
- **The Evolutions tab was hard to read, the condition lines worst.** They carry what you need to know before evolving and were the smallest text on the page. Everything is bigger now, the conditions most. Reported by iknowimgodly.
...
- **A server decides how an evolution looks, not each player.** The five sequence timings, the top spin speed, the element colours and the finale style now travel to a client with the tree, the way the costs and the stone rule already did. A server owner sets the look once on the website and every player sees it. What stays with the player: the confirm key and its timings, because a server has no business over someone's keyboard, the effect budget, because it exists for the machine drawing the frames, and the diagnostics. Leaving the server gives all of it back.
