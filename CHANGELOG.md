...
- **A rollback that could not hand your items back said so as if nothing had happened.** With a full inventory the refund fails, and the reply was the same line as an ordinary rollback. It now says the items stayed behind.
...
- **A file was written after every evolution that nothing ever read.** `palvolve_state.lua` was truncated at startup and rewritten on the game thread on every evolution and rollback, and it stayed behind after uninstalling. It is gone, and an old one is cleaned up on the next start.
